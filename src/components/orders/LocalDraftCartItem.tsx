...
interface LocalDraftCartItemProps {
    item: LocalDraftItem;
    onUpdateQty: (foodId: number, newQty: number) => void;
    onDelete: (foodId: number) => void;
}

export default function LocalDraftCartItem({
    item,
    onUpdateQty,
    onDelete,
}: LocalDraftCartItemProps) {
    const handleDecrease = () => {
        if (item.qty > 1) {
            onUpdateQty(item.food_id, item.qty - 1);
        }
    };
